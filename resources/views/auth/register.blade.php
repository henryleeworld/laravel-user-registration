                ...
                <form method="POST" action="{{ route('register') }}">
                    ...
                    <div class="input-group mb-3" id="state" style="display:none">
                        ...
                        <select class="custom-select{{ $errors->has('state_id') ? ' is-invalid' : '' }}" id="state_id" name="state_id">
                            <option value="" selected>選擇州</option>
                    ...
                    <div class="input-group mb-3" id="cityName" style="display:none">
                        ...
                        <input type="text" id="city_name" name="city_name" class="form-control{{ $errors->has('city_name') ? ' is-invalid' : '' }}" placeholder="縣市">
                        @if($errors->has('password'))
                            <div class="invalid-feedback">
                                {{ $errors->first('password') }}
                            </div>
...
$(document).ready(function() {
    $('#country_id').change(function() {
        ...
        if($('#country_id option:selected').text() == "United States") {
            $('#cityName').hide(150);
            $('#city_id').html('<option value="" selected>選擇縣市</option>');
            $('#state, #city').show(150);
        } else {
            $('#state, #city').hide(150);
            $('#cityName').show(150);
    ...
    $('#state_id').change(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                $city.html('<option value="" selected>選擇縣市</option>');
